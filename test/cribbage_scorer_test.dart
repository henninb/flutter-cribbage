...
void main() {
  group('CribbageScorer', () {
    ...
    test('flush scoring respects crib rules', () {
      ...
      final regular =
          CribbageScorer.scoreHandWithBreakdown(hand, starterHeart, false);
      expect(
      ...
      expect(
        cribNoStarter.entries.where((e) => e.type.contains('Flush')),
        isEmpty,
      );
      ...
      expect(
        cribAllMatch.entries
            .where((e) => e.type.contains('Flush'))
            .single
            .points,
        5,
      );
    ...
    test('duplicate ranks contribute multiple run combinations', () {
      ...
      final breakdown =
          CribbageScorer.scoreHandWithBreakdown(hand, starter, false);
      final runEntries =
          breakdown.entries.where((e) => e.type == 'Sequence').toList();
      expect(runEntries.length, 4); // 2x5 * 2x6 combinations
      expect(runEntries.every((entry) => entry.points == 3), isTrue);
      final totalRunPoints =
          runEntries.fold<int>(0, (sum, entry) => sum + entry.points);
    ...
    test('scores maximum hand (29 points) correctly', () {
      ...
      expect(
        ...
        28,
      ); // 4 fives + J = 28 (29 requires right nobs)
    });
    ...
    test('scores multiple fifteens correctly', () {
      ...
      expect(
        breakdown.totalScore,
        12,
      ); // 4 fifteens * 2 points + 1 run (10-J-Q-K) * 4 points
    });
    ...
    test('scores 4-card run correctly', () {
      ...
      final breakdown =
          CribbageScorer.scoreHandWithBreakdown(hand, starter, false);
      final runEntry =
          breakdown.entries.where((e) => e.type == 'Sequence').single;
    ...
    test('does not award his nobs when jack suit differs from starter', () {
      ...
      const starter =
          PlayingCard(rank: Rank.seven, suit: Suit.clubs); // Different suit
    ...
    test('4-card flush in hand scores 4 points', () {
      ...
      const starter =
          PlayingCard(rank: Rank.king, suit: Suit.hearts); // Different suit

      final breakdown =
          CribbageScorer.scoreHandWithBreakdown(hand, starter, false);
      final flushEntry =
          breakdown.entries.where((e) => e.type == 'Flush').single;
