...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **Kotlin (Android / Jetpack Compose)**: `Runtime.exec` / `ProcessBuilder` with user input, Java interop deserialization (`ObjectInputStream`), `WebView.evaluateJavascript` with untrusted content, exported `Activity` or `BroadcastReceiver` without permission checks (`android:exported="true"` without `android:permission`), hardcoded secrets in `BuildConfig`, `strings.xml`, or `local.properties`, OkHttp/Retrofit requests logging full response bodies including auth tokens in debug builds, keystore signing credentials hardcoded in `build.gradle.kts` instead of environment variables or `~/.gradle/gradle.properties`
